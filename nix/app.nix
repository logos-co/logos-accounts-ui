  ...
  preFixup = ''
    ...
    find $out -type f -executable -exec sh -c '
      if file "$1" | grep -q "ELF.*executable"; then
        ...
        if echo "$1" | grep -q "/logos-accounts-ui-app$"; then
...
Logos Accounts UI App - Build Information
=====================================
...
Runtime Layout:
- Binary: $out/bin/logos-accounts-ui-app
- Libraries: $out/lib
- Modules: $out/modules
- Qt Plugin: $out/accounts_ui.$OS_EXT
